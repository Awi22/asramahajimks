...
namespace app\models\wuling;

use app\libraries\Capsule;
use app\libraries\Datatable;
use app\modules\elo_models\wuling\mBukuBesar;
use app\modules\elo_models\wuling_as\mCustomer;
use app\modules\elo_models\wuling_as\mEstimasi;
use app\modules\elo_models\wuling_as\mLabor;
use app\modules\elo_models\wuling_as\mWorkOrder;
...
class ModelService
{
    ...
    public static function filterKodeWO()
    {
        static::$kodeWo = func_get_args();
        return new static;
    }
    ...
    private function setQuery($jenis = NULL)
    {
        ...
        if ($jenis === 'estimasi') {
            ...
            if (!empty(static::$kodeWo)) {
                $this->query = $this->query->whereKode_paket_wo(static::$kodeWo);
            }
            ...
            $this->query = mWorkOrder::with([
                ...
                'toBukuBesarByWo.toPenerimaanBengkelNoRef' => function ($query) {
                    $query->whereIn("jenis_bayar", ["t", "tf", "kk"]);
                },
                ...
                'toBukuBesarByWo.toPengeluaranBengkelNoRef.toBukuBesar' => function ($query) {
                    $query->whereIn('kode_akun', ['110102', '110201'])->whereDk('K')->whereJb('0');
                },
                ...
                'toDetailJasaBp.toHistoryProgressJasaArray.toKaryawan.toLevelMekanik',
                'toDetailPart' => function ($query) {
                    if (!empty(static::$jenisWo)) {
                        if (static::$jenisWo === 'claim') {
                            $query->whereClaim('y');
            ...
            $this->query = mBukuBesar::with([
                ...
                'toPengeluaranBengkelNoRef.toBukuBesar' => function ($query) {
                    $query->whereIn('kode_akun', ['110403'])->whereDk('K')->whereJb('0');
                },
                'toUser.toKaryawan',
                //toWorkOrderNoInv
                ...
                'toWorkOrderNoInv.toDetailUnitCustomer.toUnit',
                ...
                'toWorkOrderNoInv.toDetailJasaBp.toHistoryProgressJasaArray.toKaryawan.toLevelMekanik',
                ...
                'toInvoiceAfterSalesArray.toWorkOrder.toHutangVendorLokal',
                ...
                'toInvoiceAfterSalesArray.toWorkOrder.toDetailUnitCustomer.toUnit',
                ...
                'toInvoiceAfterSalesArray.toWorkOrder.toPaket' => function ($query) {
                    if (!empty(static::$noInvoice)) {
                        if (!is_array(static::$noInvoice)) {
                            if (strpos(static::$noInvoice, 'INV-WO-CLM.') !== FALSE || strpos(static::$noInvoice, 'INV.WO-CLM.') !== FALSE) {
                                $query->whereClaim('y');
                            } elseif (strpos(static::$noInvoice, 'INV-WO.') !== FALSE || strpos(static::$noInvoice, 'INV.WO.') !== FALSE) {
                                $query->whereClaim('n');
                            }
                        }
                    }
                ...
                'toInvoiceAfterSalesArray.toWorkOrder.toPaket.toLabor',
                ...
                'toInvoiceAfterSalesArray.toWorkOrder.toDetailJasa' => function ($query) {
                    ...
                    $query->whereDel('0');
                },'toInvoiceAfterSalesArray.toWorkOrder.toDetailJasaBp' => function ($query) {
                        if (!empty(static::$noInvoice)) {
                            if (!is_array(static::$noInvoice)) {
                                if (strpos(static::$noInvoice, 'INV-WO-CLM.') !== FALSE) {
                                    $query->whereClaim('y');
                ...
                'toInvoiceAfterSalesArray.toWorkOrder.toDetailJasaBp.toHistoryProgressJasaArray.toKaryawan.toLevelMekanik',
                ...
                // ->where('buku_besar.no_transaksi','like','%INV-%');
            if (!empty(static::$jenisInvoice)) {
            ...
            if (!empty(static::$jenisInvoice)) {
                if (static::$jenisInvoice === 'customer') {
                    $this->query = $this->query->where('buku_besar.no_transaksi', 'not like', 'INV-WO-CLM.%')->where('buku_besar.no_ref','like','%WO%');
    ...
    private function generateData($result)
    {
        ...
        foreach ($result->workOrder as $key => $value) {
            ...
            $tax                    = get_tax_ppn('ppn', $value->tgl_service);
            $cust                   = mCustomer::find($value->toCustomer->id_customer ?? $value->toCustomer['id_customer']);
            $pendapatanJasa         = 0.0;
            $pendapatanJasaBp       = 0.0;
            $pendapatanPart         = 0.0;
            $pendapatanOli          = 0.0;
            ...
            if ($value->toCustomer !== NULL) {
                ...
                if (strpos($result->no_transaksi, 'INV-WO-CLM.') !== FALSE || strpos($result->no_transaksi, 'INV.WO-CLM.') !== FALSE) {
                    $value->toCustomer->nama    = 'PT. SGMW Motor Indonesia';
                    $value->toCustomer->telepon = '(021) 80865050';
                    $value->toCustomer->alamat  = 'Greenland International Industrial Center (GIIC) Blok BA No. 1 Deltamas, Sukamahi, Kec. Cikarang Pusat, Kabupaten Bekasi, Jawa Barat 17530';
                    $value->toCustomer->npwp    = '-';
                } else {
                    if (str_contains($value->toEstimasi->kode_paket_wo, 'PDI') !== FALSE) {
                        $value->toCustomer->nama = 'PT. KUMALA CEMERLANG ABADI';
                    } else {
                        $value->toCustomer->nama = $cust->nama;
                    }
                }
                $value->toCustomer->telepon = $value->toDetailUnitCustomer->telepon_stnk;
                $value->toCustomer->alamat  = $value->toDetailUnitCustomer->alamat_stnk;
            }

            $totalJasaUtama    = 0;
            $totalJasaTambahan = 0;
            $totalJasaBodyPaint = 0;
            $totalPartUtama    = 0;
            $totalPartTambahan = 0;
            ...
            if ($value->toPaket !== NULL) {
                ...
                if (static::$allDetail !== TRUE) {
                    if (strpos($result->no_transaksi, 'INV-WO.') !== FALSE || strpos($result->no_transaksi, 'INV.WO.') !== FALSE) {
                        if ($value->toPaket->claim !== 'n') {
                            $continue = TRUE;
                        }
                    } elseif (strpos($result->no_transaksi, 'INV-WO-CLM.') !== FALSE || strpos($result->no_transaksi, 'INV.WO-CLM.') !== FALSE) {
                ...
                $value->toPaket->kode     = $value->toPaket->toLabor->maintenance_project_code;
                $value->toPaket->nama     = $value->toPaket->toLabor->maintenance_project_name;
                ...
                if ($continue === FALSE) {
                    if (static::$nilaiReal === TRUE) {
                        ...
                        $value->toPaket->ppn    = $value->toPaket->claim === 'y' ? 0.0 : round($value->toPaket->total * $tax);
                        ...
                        $value->toPaket->ppn    = round($value->toPaket->total * $tax);
            ...
            // foreach ($value->toDetailJasaBp as $key1 => $value1) {
            //     if (static::$allDetail !== TRUE) {
            //         if (strpos($result->no_transaksi, 'INV-WO.') !== FALSE || strpos($result->no_transaksi, 'INV.WO-CLM.') !== FALSE) {
            //             if ($value1->claim !== 'n') {
            //                 continue;
            //             }
            //         } elseif (strpos($result->no_transaksi, 'INV-WO-CLM.') !== FALSE || strpos($result->no_transaksi, 'INV.WO-CLM.') !== FALSE) {
            ...
            //     $value1->nama   = $value1->toJasaBp->nama_jasa;
            //     $value1->harga  = $value1->harga_jual;
            //     $value1->diskon = $value1->diskon_rp;
            //     $value1->panel    = $value1->panel;

            //     if (static::$nilaiReal === TRUE) {
            //         $value1->total     = $value1->harga - $value1->diskon;
            //         $value1->ppn       = round($value1->total * $tax);
            //         $pendapatanJasaBp += $value1->harga;
            ...
            //     $totalJasaBodyPaint += $value1->total;
            //     $jasaBp[] = $value1;
            // }
            foreach ($value->toDetailJasa as $key1 => $value1) {
                ...
                $value1->kode   = $value1->toLabor->maintenance_project_code ?? '-';
                $value1->nama   = $value1->is_utama === 1 ? 'Paket - ' . $value1->toLabor->maintenance_project_name : $value1->toLabor->maintenance_project_name;
                $value1->harga  = $value1->harga_jual;
                $value1->diskon = $value1->diskon_rp;
                ...
                if (static::$nilaiReal === TRUE) {
                    ...
                    $value1->ppn     = $value1->claim === 'y' ? 0.0 : round($value1->total * $tax);
            ...
            $value->toDetailJasaBp  = $detailJasaBp;
            ...
            $totalPpn    = round($subtotal * $tax);
            $total       = round($subtotal + $totalPpn);
            $value->nilai = (object) [
                'pendapatanJasa'         => $pendapatanJasa,
                'pendapatanJasaBp'       => $pendapatanJasaBp,
                ...
                'jasaBodyPaint' => $totalJasaBodyPaint,
        ...
        $pendapatanJasaBp = 0;
        $dpptotal     = $pendapatanJasa + $pendapatanPart + $pendapatanOpl + $pendapatanJasaBp;
        $totalDiskon  = $diskonJasa + $diskonPart + $diskonOpl;
        $subtotal     = $dpptotal - $totalDiskon;
    ...
    {
        $tax = get_tax_ppn('ppn', $result->tgl_service);
        ...
        $type = $result->toDetailUnitCustomer->toUnit->id_type;
        $getLabor = mLabor::where('id_type',$type)->select('maintenance_project_code','maintenance_project_name')->first();
        ...
        if ($result->toPaket !== NULL) {
            ...
            $result->toPaket->kode     = $getLabor->maintenance_project_code;
            $result->toPaket->nama     = $getLabor->maintenance_project_name;
            $result->toPaket->harga    = $result->toPaket->total_jasa;
            $result->toPaket->diskon   = 0;
            $result->toPaket->frt      = $result->toPaket->frt ?? $result->toPaket->toLabor->standard_working_hours;
            $result->toPaket->is_utama = 1;

            if (static::$nilaiReal === TRUE) {
                $result->toPaket->total  = $result->toPaket->claim === 'y' ? 0.0 : $result->toPaket->harga;
                $result->toPaket->ppn    = $result->toPaket->claim === 'y' ? 0.0 : round($result->toPaket->total * $tax);
                ...
                $result->toPaket->ppn    = round($result->toPaket->total * $tax);
        ...
        foreach ($result->toDetailJasa as $key1 => $value) {
            ...
            // dd($value->toLabor->maintenance_project_code);
            if ($value->claim === 'y') {
                $y = $value->claim;
            } else {
                $n = $value->claim;
            }
            $value->kode   = $value->toLabor->maintenance_project_code ?? '-';
            $value->nama   = $value->toLabor->maintenance_project_name ?? '-';
            ...
            if (static::$nilaiReal === TRUE) {
                ...
                $value->ppn      = $value->claim === 'y' ? 0.0 : round($value->total * $tax);
                ...
                $value->ppn      = round($value->total * $tax);
        ...
        foreach ($result->toDetailJasaBp as $key1 => $value) {
            if ($value->claim === 'y') {
                $y = $value->claim;
            } else {
                $n = $value->claim;
            }
            $value->kode   = $value->toJasaBp->kode_jasa;
            $value->nama   = $value->toJasaBp->nama_jasa;
            $value->harga  =  $value->harga_jual;
            $value->diskon = $value->diskon_rp;
            ...
            if (static::$nilaiReal === TRUE) {
                $value->total    = $value->claim === 'y' ? 0.0 : $value->harga - $value->diskon;
                $value->ppn      = $value->claim === 'y' ? 0.0 : round($value->total * $tax);
                $pendapatanJasaBp += $value->claim === 'y' ? 0.0 : $value->harga;
                $diskonJasa     += $value->claim === 'y' ? 0.0 : $value->diskon;
            } else {
                $value->total      = $value->harga - $value->diskon;
                $value->ppn        = round($value->total * $tax);
                $pendapatanJasaBp += $value->harga;
                $diskonJasa       += $value->diskon;
            }
            ...
            $totalJasaBodyPaint += $value->total;
            $jasaBodyPaint[] = $value;
        }
        foreach ($result->toDetailPart as $key1 => $value) {
            ...
            // untuk ambil hpp baru (average coasting)
            $value->hpptotal = get_hpp(static::$perusahaan, $value->kode_item, $value->qty);
        ...
        $dpptotal    = $pendapatanJasaBp + $pendapatanJasa + $pendapatanPart + $pendapatanOpl;
        $totalDiskon = $diskonJasa + $diskonPart + $diskonOpl;
        $subtotal    = $dpptotal - $totalDiskon;
        $totalPpn    = round($subtotal * $tax);
        $total       = round($subtotal + $totalPpn);
    ...
    private function generateDataEstimasi($result)
    {
        ...
        if ($result->toPaket !== NULL) {
            ...
            $result->toPaket->kode     = $result->toPaket->toLabor->maintenance_project_code;
            $result->toPaket->nama     = $result->toPaket->toLabor->maintenance_project_name;
            $result->toPaket->harga    = $result->toPaket->total_jasa;
            $result->toPaket->diskon   = 0;
