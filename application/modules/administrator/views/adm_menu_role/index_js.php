<script>
	$(document).ready(function() {
		...
		table_role = $("#table_role").DataTable({
			processing: true,
			ajax: {
				url: "<?= site_url('adm_menu_role/get') ?>",
			},
			language: {},
			columns: [{
					data: "role_name",
		...
		$.ajax({
			url: "<?= site_url('adm_user_global/select2_group_menu'); ?>",
			dataType: 'JSON',
	...
	$(document).on('click', '#btn-simpan', function() {
		...
		let role_id = null,
			...
			the_url = "<?= site_url('adm_menu_role/simpan'); ?>";

		if (is_update) {
			role_id = $('#btn-simpan').data('role-id');
			the_url = "<?= site_url('adm_menu_role/update'); ?>";
		}
	...
	$(document).on('click', '.btn-edit', function() {
		...
		$.ajax({
			dataType: "JSON",
			url: "<?= site_url('adm_menu_role/get_role_by_id'); ?>",
			data: {
				id: role_id,
			},
			success: function(response) {
		...
		konfirmasi('Anda yakin untuk menghapus data?')
			.then(function(e) {
				if (e.value) {
					$.ajax({
						type: "POST",
						url: "<?= site_url('adm_menu_role/hapus'); ?>",
						data: {
							id: role_id
						},
						dataType: "JSON",
		...
		konfirmasi('Anda yakin untuk menyimpan data?').then(function(e) {
			if (e.value) {
				$.ajax({
					type: "POST",
					dataType: "JSON",
					url: "<?= site_url('adm_menu_role/assign_menu_ke_role'); ?>",
					data: {
						role_id: role_id,
						group_id: group_id,
...
<script>
	...
	$('#menuList').jstree({
		...
		core: {
			data: {
				url: "<?= site_url('adm_menu_role/get_tree_by_group_menu') ?>",
				dataType: "JSON",
				data: function(node) {
					console.log(node);
					return {
		...
		$.ajax({
			url: "<?= site_url('adm_menu_role/get_checked_tree_by_role_id') ?>",
			data: {
				role_id: role_id,
			},
