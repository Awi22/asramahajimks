...
class Model_menu_role extends CI_Model
{
	...
	public function simpan_role($posts)
	{
		...
		if (!empty($posts)) {
			...
			//cek existing
			$query_cek_role = $this->db->select('*')->from('menu_role')->where('role_name', $role_name)->get();
	...
	public function update_role($posts)
	{
		...
		if (!empty($posts)) {
			...
			$this->db->trans_start();
			$this->db->update("menu_role", $data, ['id' => $role_id]);
			$this->db->trans_complete();
	...
	public function hapus_role($posts)
	{
		...
		if (!empty($posts)) {
			...
			$this->db->delete("menu_role", ['id' => $role_id]);
			$this->db->trans_complete();
			if ($this->db->trans_status() === true) {
				$status = true;
	...
	public function role_menu($group_id)
	{
		...
		// $query = $this->db->query("SELECT category_id, category_name, category_link,parent_id FROM " . $this->category . " order by parent_id, category_link");
		$query = $this->db->query("SELECT * FROM menu WHERE group_id='{$group_id}' ORDER BY group_id, parent_id, position");
		// $query_group = $this->db->query("SELECT * FROM menu_group ORDER BY id");
	...
	function get_checked_tree_by_role_id($role_id)
	{
		$checked    = [];
		$arr        = [];
		$query      = $this->db->get_where('menu_role_group', ['role_id' => $role_id]);
		foreach ($query->result() as $q) {
	...
	function assign_menu_ke_role($group_id, $role_id, $menu_id)
	{
		...
		if (!empty($group_id) && !empty($role_id)) {
			...
			$hapus_role_lama = $this->db->delete('menu_role_group', [
				'role_id'	=> $role_id,
				'group_id'	=> $group_id
			]);
			...
			if (!empty($menu_id)) {
				...
				$this->db->insert_batch("menu_role_group", $data);
			}
...
/* Location: ./admin/models/Model_menu_role.php */
