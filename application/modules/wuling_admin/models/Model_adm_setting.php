...
class Model_adm_setting extends CI_Model
{
	...
	public function upload_background()
    {
        ...
        if ($files['background_file']['name'][0] != null) {
            ...
            $hasil = $this->db_wuling->update('system_setting', ['background_login' => $folder_name . $dataInfo[0]['file_name']]); 
        }
        if ($files['background_file']['name'][1] != null) { 
            ...
            $hasil = $this->db_wuling->update('system_setting', ['background_home' => $folder_name . $dataInfo[1]['file_name']]); 
        }
		return ['status' =>$hasil, 'pesan'=>$pesan];
    }
...
/* Location: ./wuling_admin/models/Model_adm_setting.php */
