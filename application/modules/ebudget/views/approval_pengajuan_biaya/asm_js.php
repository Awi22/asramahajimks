<script>
	$(document).ready(function() {
		...
		table_approval_po = $("#table_approval_po").DataTable({
			...
			order: [],
			ajax: {
				url: "<?= site_url('approval_pengajuan_biaya_asm/get') ?>",
				data: function(data) {
			...
			columns: [
				...
				},
				{
					...
					render: function(data, type, row, meta) {
						...
						if(row.approval_sm=='1'){
							switch (row.approval_asm) {
								case '1':
								case '2':
									break;
								default:
	...
	$(document).on('click', '.btn-approve', function() {
		...
		konfirmasi('Anda yakin untuk approve?').then(function(e) {
			if (e.value) {
				$.ajax({
					type: "POST",
					url: "<?= site_url('approval_pengajuan_biaya_asm/approve'); ?>",
					data: {
						id: id
					},
					dataType: "JSON",
	...
	$(document).on('click', '.btn-reject', async function() {
		...
		if(alasan) {
			$.ajax({
				type: "POST",
				url: "<?= site_url('approval_pengajuan_biaya_asm/reject'); ?>",
				data: {
					id: id,
					alasan: alasan
				},
