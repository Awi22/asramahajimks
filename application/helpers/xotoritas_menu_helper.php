...
function child($menu_akses, $text_menu, $id, $class, $url)
{
    if (in_array($id, $menu_akses)) {
        $html = '<li><a href="' . $url . '" data-link="link-sidebar-' . $url . '" class="' . $class . '" id="' . $id . '">' . $text_menu . '</a></li>';
    } else {
        $html = null;
    }
    return $html;
...
{
    if (in_array($id, $menu_akses)) {
        $html = '<li class="' . $class . '" id="' . $id . '"><a href="' . $url . '" id="' . $id . '" data-link="link-sidebar-' . $url . '">' . $text_menu . '</a></li>';
    } else {
        $html = null;
    }
    return $html;
...
{
    $html = in_array($id, $menu_akses) ? '<li class="' . $class . '"><a href="' . $url . '" data-link="link-sidebar-' . $url . '"><i class="' . $icon . '"></i>
...
{
    $html = in_array($id, $menu_akses) ? '<li class="' . $class . '"><a href="' . $url . '" data-link="data-link="link-sidebar-' . $url . '"" class="menu-item">' . $text_menu . '</a></li>' : null;
    return $html;
}
...
function single_menu_link($menu_akses, $icon, $text_menu, $id, $class, $url)
{
    $html = in_array($id, $menu_akses) ?
        '<li class="' . $class . ' nav-item"><a data-link="link-sidebar-' . $url . '" href="' . $url . '"><i class="' . $icon . '"></i><span data-i18n="nav.dash.main" class="menu-title">' . $text_menu . '</span></a></li>' : null;
    return $html;
}

function open_nav_header($menu_akses, $text_menu)
{
    foreach ($menu_akses as $row) {
        if (str_contains($row, 'api_')) {
            $html = '<ul class="nav nav-list" style="background-image: linear-gradient( #da2327, #c71f25) !important; padding: 7px 5px 7px 14px!important;">
                        <li">
                            <i class="icon-xxx"></i>
                            <span class="menu-text" style="color: #fff !important;"> ' . $text_menu . ' </span>
                        </li>
                    </ul>';
            break;
        } else {
